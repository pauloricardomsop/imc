...
class CPFCaptchaAdapter {
  ...
  Future<ApiResponse<Uint8List>> _getCaptcha() async {
    try {
      ...
      if (element == null) ApiResponse.error(Exception('Erro ao realizar conversão'));
      final src = element!.attributes['src'];
      if (src == null) ApiResponse.error(Exception('Erro ao realizar conversão'));
  ...
  Future<ApiResponse<CPFCaptchaSituacaoModel>> _getSituacao(
      String cpf, String dataNascimento, String captcha) async {
    try {
      final response = await CPFProvider.getSituacao(cpf, dataNascimento, captcha);
      if (response.hasError) return ApiResponse.error(response.error);
      
      if (response.data.contains("Data de nascimento informada  est&aacute; divergente")) {
        return ApiResponse.error(Exception('Data de Nascimento Incorreta'));
      }
      if (response.data.contains("foi realizada corretamente. Por favor, tente novamente")) {
        return ApiResponse.error(Exception('Captcha incorreto'));
      }
      if (response.data.contains("expirou")) {
        await getCaptcha();
        return ApiResponse.error(Exception('Captcha expirado, tente novamente'));
      }
      if (response.data.contains("Os caracteres da imagem n�o foram preenchidos corretamente. Por favor, envie novamente.")) {
        await getCaptcha();
        return ApiResponse.error(Exception('Captcha inválido, tente novamente'));
      }
      
      final document = parse(response.data);
      final clConteudoDados = document.getElementsByClassName('clConteudoDados');
      final clConteudoComp = document.getElementsByClassName('clConteudoComp');
      LogManager.instance.event('consulta_cpf' , params: {'html': response.data});
      final model = CPFCaptchaSituacaoModel(
        ...
        dataNascimento: clConteudoDados[2].text.split('Data de Nascimento: ').last,
        situacaoCadastral: clConteudoDados[3].text.split('Situação Cadastral: ').last,
        dataInscricao: clConteudoDados[4].text.split('Data da Inscrição: ').last,
        digitoVerificador: clConteudoDados[5].text.split('Digito Verificador: ').last,
        comprovanteEmitido: clConteudoComp[0].text.split('Comprovante emitido às: ').last,
        codigoControleComprovante:
            clConteudoComp[1].text.split('Código de controle do comprovante: ').last,
      );
